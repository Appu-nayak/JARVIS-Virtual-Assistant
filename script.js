...
async function playYouTubeSong(songName) {
    try {
        ...
        speak("Sorry, I encountered an error while searching for the song.");
    }
}
...
function takeCommand(message) {
    const calculate = (msg) => {
        try {
            let operation = msg.replace(/what is |calculate|\?/gi, '').trim();
    ...
    if (calculationResult !== null) {
        ...
        if (songName) {
            ...
            speak("Please specify the song name, sir.");
        }
